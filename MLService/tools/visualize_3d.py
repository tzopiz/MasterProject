...
def visualize_3d(
    ...
    z_norm_bg = (face_centers_bg[:, 2] - face_centers_bg[:, 2].min()) / (
        face_centers_bg[:, 2].max() - face_centers_bg[:, 2].min() + 1e-8
    )
    ...
    z_norm_tmj = (face_centers_tmj[:, 2] - face_centers_tmj[:, 2].min()) / (
        face_centers_tmj[:, 2].max() - face_centers_tmj[:, 2].min() + 1e-8
    )

    cmap_tmj = LinearSegmentedColormap.from_list(
        "tmj",
        [
    ...
    plot_cube(
        ax, (xr, yr, max_z - zr), crop_size, color="#00FF88", linewidth=5
    )  # Bright neon green
...
def main():
    ...
    parser.add_argument(
        "--summary",
        type=str,
        default="data/auto_crops/crop_summary.json",
        help="Path to crop summary",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Downsample factor (increase for speed, decrease for quality)",
    )
    parser.add_argument(
        "--threshold", type=float, default=None, help="Bone threshold (HU/intensity)"
    )
    ...
    views = [
        (i * 40, f"{i * 40:03d}deg") for i in range(9)
