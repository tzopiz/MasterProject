                ...
                for series_folder in date_folder.iterdir():
                    ...
                    if dcm_files:
                        ...
                        try:
                            ...
                            study_info["metadata"] = {
                                "modality": convert_dicom_value(
                                    getattr(first_dcm, "Modality", "Unknown")
                                ),
                                "series_description": convert_dicom_value(
                                    getattr(first_dcm, "SeriesDescription", "Unknown")
                                ),
                                "manufacturer": convert_dicom_value(
                                    getattr(first_dcm, "Manufacturer", "Unknown")
                                ),
                            }
                        except Exception as e:
                            logger.warning(
                                f"Could not read DICOM metadata from {dcm_files[0]}: {e}"
                            )
                            study_info["metadata"] = {}

                        studies.append(study_info)
                        logger.info(
                            f"  Found study: {study_info['study_id']} ({len(dcm_files)} files)"
                        )
