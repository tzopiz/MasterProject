...
def plot_prediction(volume, coords_left, coords_right, crop_size, output_path, manual_left=None, manual_right=None):
    ...
    title = f'TMJ Detection (Red=Model, Green=Manual)\nCrop Size: {crop_size}x{crop_size}x{crop_size}'
    fig.suptitle(title, fontsize=16)
    
    # Helper to draw box
    def draw_box(ax, center_r, center_c, color, label=None):
        r, c = center_r, center_c
        rect = patches.Rectangle((c - h, r - h), crop_size, crop_size, linewidth=2, edgecolor=color, facecolor='none', label=label)
        ax.add_patch(rect)
        ax.plot(c, r, color + '+')
...
def main():
    ...
    parser.add_argument('--crops', type=str, default='data/auto_crops', help='Path to crop metadata')
    ...
    plot_prediction(volume, coords_left, coords_right, crop_size, output_file, manual_left, manual_right)
