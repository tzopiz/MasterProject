...
def evaluate(model, loader, device, ds_factor, voxel_mm=None):
    ...
    if voxel_mm:
        ...
        results["pct_within_5mm"] = 100 * float(
            np.mean([e * ds_factor * voxel_mm < 5 for e in all_errs])
        )
        results["pct_within_10mm"] = 100 * float(
            np.mean([e * ds_factor * voxel_mm < 10 for e in all_errs])
        )
