...
class CropExtractor:
    ...
    def __init__(self, annotations_dir: str, dataset_dir: str, output_dir: str, crop_size: int = 128):
    ...
    def extract_crop(self, volume: np.ndarray, center: List[int], crop_size: int) -> Tuple[np.ndarray, List[int]]:
        """
        ...
        if crop.shape != (crop_size, crop_size, crop_size):
            ...
            crop = padded_crop
            logger.warning(f"  Padded crop from {(z_end-z_start, y_end-y_start, x_end-x_start)} to {crop.shape}")
    ...
    def process_annotation(self, annotation_file: Path):
        ...
        try:
            ...
            right_crop, right_actual_center = self.extract_crop(volume, right_center, self.crop_size)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help='Size of cubic crop (default: 128)'
    )
