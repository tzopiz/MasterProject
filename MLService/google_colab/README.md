...
| [train_sagittal_binary_cv.ipynb](train_sagittal_binary_cv.ipynb) | **Сагитталь, CV:** обёртка над `training/sagittal_binary_cv.py`. **DataSphere:** `tmj` в `/home/jupyter/datasets/tmj/`, кропы `detector_crops_v2`; манифест — `manifest_private.json` или `manifest.json` в датасете, иначе поиск в `/home/jupyter/filestore/`. Переменные: `TMJ_MANIFEST_PATH`, `TMJ_LABELS_PATH`, `TMJ_DATASET_DIR`. JSON отчёта — `filestore/experiments/`. Инициализация: [init_datasphere_dataset.ipynb](init_datasphere_dataset.ipynb). |
