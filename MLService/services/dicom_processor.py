...
class DICOMProcessor:
    ...
    def load_series(self, directory_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Series loaded: {pixel_array.shape}, Spacing: {pixel_spacing}, Thickness: {slice_thickness}"
            )
            return dicom_data
    ...
    def load_dicom(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"DICOM loaded successfully: {metadata['modality']}, shape: {pixel_array_normalized.shape}"
            )
            return dicom_data
        ...
        try:
            ...
            if len(pixel_array.shape) == 3:
                if 0 <= slice_index < pixel_array.shape[0]:
                    ...
                    logger.error(
                        f"Slice index {slice_index} out of range (0-{pixel_array.shape[0] - 1})"
                    )
                    return None
