...
class DiagnosisEngine:
    ...
    def diagnose(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if width is not None:
                status, conf = self._check_parameter(
                    "ширина сустава", width, self.NORMAL_RANGES["width"]
                )
                if status != "normal":
                    abnormalities.append(status)
                    confidence_scores.append(conf)
            ...
            if len(abnormalities) == 0:
                ...
                confidence = (
                    sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0.7
                )
                recommendations = self._get_minor_abnormality_recommendations(abnormalities)
            else:
                overall_status = "abnormal"
                confidence = (
                    sum(confidence_scores) / len(confidence_scores) if confidence_scores else 0.75
                )
    ...
    def _check_parameter(
        self, param_name: str, value: float, normal_range: tuple
    ) -> tuple[str, float]:
        """
    ...
    def calculate_asymmetry(
        self, left_params: Dict[str, float], right_params: Dict[str, float]
    ) -> Dict[str, Any]:
        """
