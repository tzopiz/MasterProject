...
from validation.visualize import plot_side_by_side
...
def evaluate_pair(
    model: UNet, vol_path: Path, mask_path: Path, device: torch.device, output_dir: Path
) -> Dict:
    """
...
def evaluate_model(model_path: Path, data_dir: Path, output_dir: Path):
    ...
    # Average metrics
    avg_metrics = {}
    metric_keys = ["dice", "iou", "precision", "recall", "specificity", "volume_dice", "volume_iou"]
    ...
    print(
        f"  Average Dice:       {avg_metrics.get('dice', 0):.4f} ± {avg_metrics.get('dice_std', 0):.4f}"
    )
    print(
        f"  Average IoU:        {avg_metrics.get('iou', 0):.4f} ± {avg_metrics.get('iou_std', 0):.4f}"
    )
    print(
        f"  Average Precision:  {avg_metrics.get('precision', 0):.4f} ± {avg_metrics.get('precision_std', 0):.4f}"
    )
    print(
        f"  Average Recall:     {avg_metrics.get('recall', 0):.4f} ± {avg_metrics.get('recall_std', 0):.4f}"
    )
    print(
        f"  Volume-level Dice:  {avg_metrics.get('volume_dice', 0):.4f} ± {avg_metrics.get('volume_dice_std', 0):.4f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="validation/reports/latest",
        help="Output directory for reports",
    )

    args = parser.parse_args()

    evaluate_model(Path(args.model), Path(args.data), Path(args.output))
