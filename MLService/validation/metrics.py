...
def hausdorff_distance(pred: np.ndarray, target: np.ndarray, percentile: int = 95) -> float:
    ...
    if percentile < 100:
        ...
        distances_pred = np.min(np.linalg.norm(
            pred_points[:, None] - target_points[None, :], axis=2), axis=1)
        distances_target = np.min(np.linalg.norm(
            target_points[:, None] - pred_points[None, :], axis=2), axis=1)
...
def average_surface_distance(pred: np.ndarray, target: np.ndarray, 
                            spacing: Tuple[float, ...] = None) -> float:
    """
