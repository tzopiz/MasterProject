...
def train_epoch(model, dataloader, criterion_bce, criterion_dice, optimizer, device):
    ...
    with tqdm(dataloader, desc="Training", unit="batch") as pbar:
        for volumes, masks in pbar:
            ...
            pbar.set_postfix(
                {"loss": f"{loss.item():.4f}", "dice": f"{(1.0 - loss_dice.item()):.4f}"}
            )
...
def main(args):
    ...
    model = UNet3D(in_channels=1, out_channels=1, init_features=args.init_features).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        # Validate
        val_loss, val_dice = validate(model, val_loader, criterion_bce, criterion_dice, device)
...
if __name__ == "__main__":
    ...
    # Data
    parser.add_argument(
        "--data_dir", type=str, default="data/processed_crops", help="Path to dataset directory"
    )
    parser.add_argument("--val_split", type=float, default=0.2, help="Validation split ratio")

    # Model
    parser.add_argument(
        "--init_features", type=int, default=16, help="Initial number of features (16 or 32)"
    )
    parser.add_argument(
        "--crop_size", type=int, default=128, help="Crop size (128 or 96 for less memory)"
    )

    # Training
    parser.add_argument("--epochs", type=int, default=100, help="Number of epochs")
    parser.add_argument(
        "--batch_size", type=int, default=2, help="Batch size (2 for 3D, may need 1 if OOM)"
    )
