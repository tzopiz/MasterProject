...
class TMJDetectorDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f"TMJDetectorDataset: {len(self.annotations)} samples ({'train' if is_train else 'val'})")
    ...
    def _denormalize_coordinates(self, normalized_coords: np.ndarray, downsampled_shape: np.ndarray) -> np.ndarray:
        """Convert normalized coordinates back to downsampled volume space"""
        return normalized_coords * downsampled_shape
    
    def _augment_volume(self, volume: np.ndarray, left_coords: np.ndarray, right_coords: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def __getitem__(self, idx):
        ...
        downsampled_shape = np.array(volume.shape)
        ...
        # Augmentation
        if self.augment:
            volume, left_coords, right_coords = self._augment_volume(volume, left_coords, right_coords)
