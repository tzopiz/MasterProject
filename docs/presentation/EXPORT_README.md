...
1. Откройте `docs/presentation/presentation.html` в Chrome или Safari (двойной клик по файлу или перетащите в окно браузера).
...
python3 scripts/presentation/export_presentation_pdf.py
```

Результат: `docs/presentation/presentation.pdf`.

### PPTX (каждая секция — слайд)

```bash
source .venv_export/bin/activate
python3 scripts/presentation/export_presentation_pptx.py
```

Результат: `docs/presentation/presentation.pptx` (5 слайдов: титул, проблема/решение, возможности, метрики, рынок и CTA).
